...
do
    ...
    if [ -n "$VETO" ]; then
        eval "./bin/madwizard.js json ./test/inputs/$i/in.md -O0 $ASSERT $VETO > ./test/inputs/$i/wizard-veto.txt" &
    fi
...
do
    ...
    eval "./bin/madwizard.js run ./test/inputs/$i/in.md $ASSERT --verbose --no-profile > ./test/inputs/$i/run.txt" &
...
do
    ...
    do
        ...
        eval "./bin/madwizard.js run ./test/inputs/$i/in.md $ASSERT $ASSERT2 --verbose --no-profile > ./test/inputs/$i/run-$platform.txt" &

        if [ -n "$VETO" ]; then
            eval "./bin/madwizard.js json ./test/inputs/$i/in.md -O0 $ASSERT $ASSERT2 $VETO > ./test/inputs/$i/wizard-veto.txt" &
        fi
    done
    wait
    echo -n .
