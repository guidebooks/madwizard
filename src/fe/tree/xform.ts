...
export default class TreeTransformer<Content> {
  ...
  private foldImportsIntoWizards(children: UITree<Content>, depth = 0): UITree<Content> {
    const { subtasks, titledSteps, rest } = children.reduce(
      ...
      {
        subtasks: [] as UITree<Content>,
        titledSteps: [] as UITree<Content>,
        rest: [] as UITree<Content>,
      }
    )
   ...
   */
  public foldChoices(children: UITree<Content>): UITree<Content> {
   ...
   */
  public foldSingletonSubTask(data: UITree<Content>) {
    /* if (data.length === 1) {
  ...
  public optimize(children: UITree<Content>, depth = 0): UITree<Content> {
    return this.foldImportsIntoWizards(this.foldChoices(children), depth)
  }
}
